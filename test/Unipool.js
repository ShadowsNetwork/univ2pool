...
const Dows = artifacts.require('DOWSMock');
const Univ2pool = artifacts.require('Univ2poolMock');
...
contract('Univ2pool', function ([_, wallet1, wallet2, wallet3, wallet4]) {
    describe('Univ2pool', async function () {
        beforeEach(async function () {
            this.uni = await Uni.new();
            this.dows = await Dows.new();
            this.pool = await Univ2pool.new(this.uni.address, this.dows.address);

            await this.pool.setRewardDistribution(wallet1);

            await this.dows.mint(this.pool.address, web3.utils.toWei('1000000'));
